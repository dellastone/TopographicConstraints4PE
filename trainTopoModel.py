...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch=None, max_epochs=None):
    ...
    for images, heatmaps in loop:
        ...
        running_ws += ws_loss.item() * images.size(0)
        running_pckh += pckh[1] * images.size(0)

        loop.set_postfix(train_loss=f"{running_loss / ((loop.n + 1) * loader.batch_size):.4f}", mse=f"{running_mse / ((loop.n + 1) * loader.batch_size):.4f}", ws=f"{running_ws / ((loop.n + 1) * loader.batch_size):.4f}", spatial_lambda=f"{spatial_lambda:.4f}", train_pckh=f"{running_pckh / ((loop.n + 1) * loader.batch_size):.4f}")


    return running_loss / len(loader.dataset), running_mse / len(loader.dataset), running_ws / len(loader.dataset), running_pckh / len(loader.dataset)
...
def validate(model, loader, criterion, device, epoch=None, max_epochs=None, wandblog: bool = False, save_plots: bool = False, save_dir: str = None):
    ...
    with torch.no_grad():
        for images, heatmaps in loop:
            ...
            running_loss += loss.item() * images.size(0)
            running_mse += mse_loss.item() * images.size(0)
            running_spatial += ws_loss.item() * images.size(0)
            running_pckh += pckh[1] * images.size(0)
...
def add_argument_parser():
    ...
    parser.add_argument('--spatial-lambda', type=float, default=0.5, help="λ weight for spatial loss.")
    parser.add_argument('--topo-grid', type=int, nargs=2, default=[16, 16],
                        help="Gh Gw grid for topographic layer (Gh*Gw should ideally be 256).")
    ...
    parser.add_argument('--num-keypoints', type=int, default=16, help='Number of keypoints (e.g., MPII=16)')
...
def main():
    ...
    if SAVE_PLOTS:
        headers = ["train_loss", "val_loss", "train_pckh", "val_pckh", "train_mse", "val_mse", "train_ws", "val_ws", "moransI_strength","contiguity"]
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"\n=== Epoch {epoch}/{EPOCHS} ===")
        train_loss, train_mse, train_ws, train_pckh = train_one_epoch(model, train_loader, Criterion, optimizer, device, epoch, EPOCHS)
        model.eval()
        val_loss, val_mse, val_ws, val_pckh = validate(model, val_loader, Criterion, device, epoch, EPOCHS, wandblog=WANDBLOG, save_plots=SAVE_PLOTS, save_dir=os.path.join("plots", run_name) if SAVE_PLOTS else None)
        ...
        #log on wandb
        wandbLossLog = {"train_loss": train_loss, "val_loss": val_loss, "train_pckh": train_pckh, "val_pckh": val_pckh, "train_mse": train_mse, "val_mse": val_mse, "train_ws": train_ws, "val_ws": val_ws}
        if WANDBLOG:
        ...
        if WANDBLOG or SAVE_PLOTS:
            ...
            pref_t = topo_analysis.plot_keypoint_parcellation_from_head(model, title="Keypoint Parcellation", grid_hw=TOPO_GRID, topo=True, epoch=epoch,
                                                                        wandblog=WANDBLOG, save_plots=SAVE_PLOTS, save_dir=metrics_dir)

            topo_analysis.plot_coactivation_distance_curve(model, val_loader, device, topo=True, grid_hw=TOPO_GRID, epoch=epoch,
                                                           wandblog=WANDBLOG, save_plots=SAVE_PLOTS, save_dir=metrics_dir)
            topo_analysis.plot_activation_neighbor_corr(model.topo, model, val_loader, device, topo=True, title_prefix="Classic activation neighbor corr", epoch=epoch,
                                                        wandblog=WANDBLOG, save_plots=SAVE_PLOTS, save_dir=metrics_dir)
            ...
            if SAVE_PLOTS:
                with open(metrics_csv, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([epoch, train_loss, val_loss, train_pckh, val_pckh, train_mse, val_mse, train_ws, val_ws, I_t, P_c]) 
